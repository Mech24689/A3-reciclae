...
// -------------------------------------------------------------------------
// Rota: POST /usuarios/create-from-pessoa (Comunicação MS-to-MS)
// -------------------------------------------------------------------------
export async function createUsuarioFromPessoa(req: Request, res: Response): Promise<void> {
    const { pessoaId, email, senha_texto_puro, role, prefeituraId } = req.body;
    ...
    const usuarioPayload: UsuarioCreate = {
        ...
        senha_texto_puro: senha_texto_puro, 
        role: role, 
        prefeitura_id: prefeituraId || 1, 
        pessoa_id: pessoaId,
    };

    try {
        await usuarioService.createUsuario(usuarioPayload);
        console.log(`[MS-Usuário] Conta criada com sucesso para ${email} (via HTTP).`);
        res.status(201).json({ message: 'Usuário criado com sucesso.' });
    } catch (error: any) {
        console.error(`[MS-Usuário] Erro ao criar usuário (via HTTP):`, error);
        res.status(500).json({ message: 'Falha interna ao criar usuário.', details: error.message });
    }
}